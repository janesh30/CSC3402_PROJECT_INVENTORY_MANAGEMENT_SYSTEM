...
public class InventoryController {
    ...
    public String viewDashboard(
            ...
            @RequestParam(value = "categoryId", required = false) Long categoryId) { // Fix 1: Matched to the HTML form

        // 1. Fetch the searched items (Fix 2: Pass BOTH variables to the service!)
        java.util.List<Ingredient> ingredients = inventoryService.searchIngredients(keyword, categoryId);
        model.addAttribute("ingredients", ingredients);

        // 2. Keep the search terms in the box after clicking search
        model.addAttribute("keyword", keyword);
        model.addAttribute("selectedCategoryId", categoryId);

        // --- THE FIX: ADD MISSING THYMELEAF VARIABLES ---
        // Pass categories for the dropdown to prevent a null crash
        model.addAttribute("categories", inventoryService.getAllCategories());

        // Pass default pagination variables so the 'totalPages > 1' check doesn't crash
        model.addAttribute("totalPages", 1);
        model.addAttribute("currentPage", 1);
        // ------------------------------------------------

        // 3. Safely calculate chart numbers in Java
        long lowCount = 0;
        long optimalCount = 0;
        ...
        // 4. Send the raw numbers to the HTML
    ...
    public String showNewIngredientForm(Model model) {
        ...
        model.addAttribute("categories", inventoryService.getAllCategories());
        model.addAttribute("vendors", inventoryService.getAllVendors()); // <-- ADD THIS
